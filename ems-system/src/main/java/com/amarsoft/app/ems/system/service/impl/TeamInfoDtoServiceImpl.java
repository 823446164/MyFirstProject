...
public class TeamInfoDtoServiceImpl implements TeamInfoDtoService {
    ...
    public TeamInfoDtoQueryRsp updateStatus(@Valid TeamInfoDtoQueryReq teamInfoDtoQueryReq) {
        ...
        if (OrgStatus.Disabled.id.equals(status)) { // 操作停用
            ...
            if( OrgStatus.Completed.id.equals(teamStatus)) {
                throw new ALSException("EMS6013");
                }
        }
        /*
         * BusinessObjectAggregate<BusinessObject> selectBusinessObjectsBySql =
         * bomanager.selectBusinessObjectsBySql(
         * "select  UT.userId  from  UserTeam UT ,TeamInfo TI where TI.teamId = UT.teamId",
         * teamInfo.getTeamId()); List<BusinessObject> businessObjects =
         * selectBusinessObjectsBySql.getBusinessObjects();
         */
    ...
    public TeamInfoDtoRoleRsp queryRole(@Valid TeamInfoDtoQueryReq request) {
        ...
        // 当前部门下的员工
        List<BusinessObject> bo = bomanager.selectBusinessObjectsBySql(
            "select  UT.userId  from  UserTeam UT" + "                       +  join TeamInfo TI on TI.teamId = UT.teamId and TI.belongOrgId =:belongOrgId and  not exists(select 1 from TeamInfo TI"
        ...
        if (!CollectionUtils.isEmpty(bo)) {
            ...
            for (BusinessObject bus : bo) {
                String userId = bus.getString("userId");
                list.add(userId);
                // BusinessObjectAggregate<BusinessObject> selectBusinessObjectsBySql =
                // bomanager.selectBusinessObjectsBySql("select UT.userId from TeamuserId
                // UT,EmployeeInfo EI where EI.employeeNO =userId",)
